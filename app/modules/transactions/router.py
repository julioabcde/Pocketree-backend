...
async def daily_summary(
    ...
    if month is not None:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Provide either 'month' or both 'start_date' and 'end_date'",
        )
...
async def update(
    ...
    if data.account_id is not None:
        new_account = await get_account_by_id(db, data.account_id)
        ...
        if new_account.user_id != transaction.user_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
